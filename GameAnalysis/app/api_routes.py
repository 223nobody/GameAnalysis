...
def load_game_data():
    """加载游戏数据和预测模型"""
    global agg_data, death_data, prediction_model
    try:
        # 获取数据文件路径（相对于app文件夹）
        data_dir = os.path.join(os.path.dirname(__file__), '..', 'data')

        # 加载聚合统计数据
        agg_data = pd.read_csv(os.path.join(data_dir, 'agg_match_stats_1.csv'))
        agg_data.drop_duplicates(inplace=True)
        agg_data['won'] = agg_data['team_placement'] == 1
        agg_data['drove'] = agg_data['player_dist_ride'] != 0

        # 加载击杀数据
        death_data = pd.read_csv(os.path.join(data_dir, 'kill_match_stats_final_1.csv'))
        ...
        result = {
            'erangel': {
                'sniper': erangel_kills.loc[
                    (erangel_kills['distance'] > 800) & (erangel_kills['distance'] < 1500),
                    'killed_by'
                ].value_counts()[:10].to_dict(),
                'melee': erangel_kills.loc[
                    erangel_kills['distance'] < 10,
                    'killed_by'
                ].value_counts()[:10].to_dict()
            },
            'miramar': {
                'sniper': miramar_kills.loc[
                    (miramar_kills['distance'] > 800) & (miramar_kills['distance'] < 1500),
                    'killed_by'
                ].value_counts()[:10].to_dict(),
                'melee': miramar_kills.loc[
                    miramar_kills['distance'] < 10,
                    'killed_by'
                ].value_counts()[:10].to_dict()
            }
        }
...
def predict_win_probability():
    ...
    try:
        ...
        try:
            ...
            record_id = db.save_prediction(data, result, tactics=tactics)
            result['record_id'] = record_id
            result['tactics'] = tactics
            print(f"预测记录已保存到数据库，ID: {record_id}, 策略: {tactics}")
